# 다음 세션 인계 — 설사·변비 발현소견 검토표 (2026-08-12 **15라운드** 종료 시점)

> 이 파일부터 읽고 시작한다. 이전 세션 전문은 `~/main/logs/`, 요약은 `docs/worklog.md` 맨 끝,
> 검수 이력 정본은 `docs/verification-log.yaml`(**15라운드까지** 기록됨).
...
- 설사 20카드 소견70·조건23·참고94 / 변비 15카드 소견52·조건15·참고52 = **306건, 인용 대조 306/306 통과 · 구조 경고 0건**
- 바탕화면 사본 = `CPX_설사변비_온톨로지_260811/15_설사_*_v7.*`, `16_변비_*_v7.*` (v4~v6도 나란히 보존)

## 🚨 최우선 — Codex 12차 검수

원문 = 바탕화면 `Codex검수11_luna.md`(4~11차 전부 보존. 입력 브리프도 각각)

**11차 판정 REVISE — BLOCKER 2 · MAJOR 14 · MINOR 8. 15라운드에서 전량 반영했다.**
**12차 검수가 다음 시작점이다.**

### 🔴 항생제 규칙 — 3차부터 11차까지 **여덟 번** 고쳤다
① 혈변만으로 항생제 → ② 전면 금지(치료 지연) → ③ 병렬 조건 → ④ 4단계 우선순위 → ⑤ 환자 3분기
→ ⑥ A/B 겹침 해소 → ⑦ 장기기능장애를 감염성 패혈증으로 한정 → ⑧ **STEC 판정을 첫 관문으로**(현재).
11차 지적이 결정적이었다: **HUS·AKI·저혈압이 있으면 STEC 환자가 '패혈증' 갈래로 올라가 항생제가
재투여된다. qSOFA는 예후 지표이지 패혈증 진단 기준이 아니며 탈수·HUS로도 올라간다.**
→ STEC 의심을 1단계로 올리고 "HUS·AKI가 있어도 회피 고정, 별도의 치료 가능한 감염원이 명확할 때만 예외".
**매 라운드 새 각도가 나온다. LLM 왕복으로 수렴하지 않는다 — 임상 권위자가 확정해야 한다.**

### ⭐ 반복되는 설계 결함 하나 — "발현소견에 발현소견이 아닌 것이 섞인다"
9차: 탈수·쇼크가 모든 질환에 중복 연결 → 【공통 경로】 카드로 분리.
11차: HUS·검사값(Hb/ESR/WBC/Cr)·가족력이 red_flag에 섞여 **"HUS나 빈혈만으로 원인 질환이 연결"**
→ `kind=complication` 신설, **관찰 가능한 소견만 red_flag**로 남기고 진단명·검사값은 참고항목으로.
**새 항목을 넣을 때마다 이 축을 확인할 것.**

### ⭐ 도구 — `findings_review.py`가 구조 모순을 잡는다
같은 축의 인용 중복 · `required_없음_사유`와 required 공존 · `해당없음` 축 충돌 · required와 사유 동시 부재.
**수정 스크립트를 돌린 뒤 반드시 확인.** 이번 세션에서 실제로 14건을 잡았다.
