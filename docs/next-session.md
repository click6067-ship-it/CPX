# 다음 세션 인계 — 설사·변비 발현소견 검토표 (2026-08-11 **11라운드** 종료 시점)

> 이 파일부터 읽고 시작한다. 이전 세션 전문은 `~/main/logs/`, 요약은 `docs/worklog.md` 맨 끝,
> 검수 이력 정본은 `docs/verification-log.yaml`(**11라운드까지** 기록됨).
...
- 설사 소견90·조건18·참고92 / 변비 소견56·조건7·참고48 = **311건, 인용 대조 311/311 통과 · 구조 경고 0건**
- 바탕화면 사본 = `CPX_설사변비_온톨로지_260811/15_설사_*_v7.*`, `16_변비_*_v7.*` (v4~v6도 나란히 보존)

## 🚨 최우선 — Codex 9차 검수

원문 = 바탕화면 `Codex검수8_luna.md`(4~8차 전부 보존. 입력 브리프도 각각)

**8차 판정 REVISE — BLOCKER 2 · MAJOR 18 · MINOR 5. 11라운드에서 전량 반영했다.**
**9차 검수는 아직 돌리지 않았다 — 거기서부터 시작하면 된다.**

### 🔴 가장 중요한 사실 — 8회 연속 REVISE, 항생제 규칙은 네 번 고쳤다
① 혈변만으로 항생제(3차) → ② STEC 배제를 전면 금지로(6차, 치료 지연) → ③ 병렬 조건(7차, 같은 환자가
양쪽 충족) → ④ 4단계 우선순위(7차 수정) → ⑤ **환자 3분기**(8차 수정, 현재).
8차 지적: "'STEC 배제 후'는 **검사 전에는 실행 불가능**하고 패혈증 즉시 치료 경로와 겹친다."
그래서 조건이 아니라 **쇼크 / STEC 고위험 / 그 밖**의 환자 분기로 다시 썼다.
**이 한 규칙은 LLM 왕복으로 수렴하지 않는다. 임상 권위자가 확정해야 한다.**
