# 다음 세션 인계 — 설사·변비 발현소견 검토표 (2026-08-11 **12라운드** 종료 시점)

> 이 파일부터 읽고 시작한다. 이전 세션 전문은 `~/main/logs/`, 요약은 `docs/worklog.md` 맨 끝,
> 검수 이력 정본은 `docs/verification-log.yaml`(**12라운드까지** 기록됨).
...
- 설사 20카드 소견72·조건24·참고88 / 변비 15카드 소견53·조건15·참고50 = **302건, 인용 대조 302/302 통과 · 구조 경고 0건**
- 바탕화면 사본 = `CPX_설사변비_온톨로지_260811/15_설사_*_v7.*`, `16_변비_*_v7.*` (v4~v6도 나란히 보존)

## 🚨 최우선 — Codex 10차 검수

원문 = 바탕화면 `Codex검수9_luna.md`(4~9차 전부 보존. 입력 브리프도 각각)

**9차 판정 REVISE — BLOCKER 2 · MAJOR 15 · MINOR 6. 12라운드에서 전량 반영했다.**
**10차 검수는 아직 돌리지 않았다 — 거기서부터 시작하면 된다.**

### ⭐ 12라운드의 핵심 — 【공통 경로】 카드 신설
Codex 9차의 가장 중요한 지적: **탈수·쇼크가 모든 질환 카드에 red_flag로 중복 연결돼
그래프상 "쇼크 하나로 아무 질환이나 지지"된다.** 온톨로지로 사례를 생성하는 이 프로젝트의 목적에
직접 해가 되는 설계 결함이다.
→ **【공통 경로】 카드 2장**(설사: 중증도·응급 처치 / 변비: 경보평가·분변매복)으로 빼고
질환 카드에서 **설사 37건·변비 6건 제거**. 각 카드에는 포인터만 남겼다.
**부수 효과: 여섯 번 고친 항생제 규칙이 한 곳에만 존재하게 되어 카드 간 불일치가 원천 차단된다.**

### 🔴 항생제 규칙 — 3차부터 9차까지 **여섯 번** 고쳤다
① 혈변만으로 항생제 → ② STEC 배제를 전면 금지(치료 지연) → ③ 병렬 조건(같은 환자가 양쪽 충족)
→ ④ 4단계 우선순위 → ⑤ 환자 3분기 → ⑥ **A/B 상호배타화 + IDSA 예외 명시**(현재).
매번 다른 각도의 결함이 나왔다. **LLM 왕복으로 수렴하지 않는다 — 임상 권위자가 확정해야 한다.**

### ⭐ 도구 — `findings_review.py`가 구조 모순도 잡는다
같은 축의 인용 중복 · `required_없음_사유`와 required 공존 · `해당없음` 축 충돌 · required와 사유 동시 부재.
**수정 스크립트를 돌린 뒤 반드시 이 경고를 확인할 것.**
