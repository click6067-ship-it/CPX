# 다음 세션 인계 — 설사·변비 발현소견 검토표 (2026-08-12 **16라운드** 종료 시점)

> 이 파일부터 읽고 시작한다. 이전 세션 전문은 `~/main/logs/`, 요약은 `docs/worklog.md` 맨 끝,
> 검수 이력 정본은 `docs/verification-log.yaml`(**16라운드까지** 기록됨).
...
- 설사 20카드 소견67·조건23·참고98 / 변비 15카드 소견49·조건16·참고62 = **315건, 인용 대조 315/315 통과 · 구조 경고 0건**
- 바탕화면 사본 = `CPX_설사변비_온톨로지_260811/15_설사_*_v7.*`, `16_변비_*_v7.*` (v4~v6도 나란히 보존)

## 🚨 최우선 — Codex 13차 검수

원문 = 바탕화면 `Codex검수12_luna.md`(4~12차 전부 보존. 입력 브리프도 각각)

**12차 판정 REVISE(SHIP 불가) — BLOCKER 2 · MAJOR 17 · MINOR 5. 16라운드에서 반영했다.**
**13차 검수가 다음 시작점이다.**

### 🔴 항생제 규칙 — 3차부터 12차까지 **아홉 번** 고쳤다
① 혈변만으로 항생제 → ② 전면 금지 → ③ 병렬 조건 → ④ 4단계 → ⑤ 환자 3분기 → ⑥ A/B 겹침 해소
→ ⑦ 감염성 패혈증으로 한정 → ⑧ STEC를 1단계로 → ⑨ **0단계 소생술 + 패혈증 예외 복원**(현재).
12차 지적이 양쪽을 다 잡았다: **⑧은 쇼크 환자에서 원인 감별을 소생술보다 앞세웠고**,
동시에 **"HUS·AKI가 있어도 판단은 안 바뀐다"가 실제 패혈증에서 생명구조 항생제를 보류**하게 했다.
→ 0단계에 무조건 ABC·수액, 검사는 동시. 항균제는 ⓐSTEC/HUS만 있고 패혈증 증거 없으면 회피 /
ⓑ패혈증·균혈증·장외감염 의심이면 **보류하지 않음** / ⓒ그 밖은 원칙적 비투여 + 정의된 예외.
**매 라운드 새 각도가 나온다. 임상 권위자가 확정해야 한다.**

### ⭐ 반복 결함 — "발현소견에 발현소견이 아닌 것이 섞인다" (세 번째 대청소)
9차: 탈수·쇼크가 모든 질환에 중복 → 【공통 경로】 카드.
11차: HUS·검사값이 red_flag에 → `kind=complication` 신설.
12차: **상태 진단명이 대거 잔존** → 쇼크·장폐색·분변매복·감돈·점액부종혼수·중증 고칼슘/저칼륨·
마미증후군·크론 협착/누공을 전부 `complication`으로 옮기고 **관찰 소견만** 남겼다.
검사값(빈혈·혈색소·ESR)은 `workup`, 가족력·연령은 배경조건, 약물명은 복약력으로.
**새 항목을 넣을 때마다 "이게 환자에게서 관찰되는가"를 먼저 물을 것.**

### ⚠️ 3차 BLOCKER가 12차에 재지적됐다
배변협조장애의 회장루. "바이오피드백 실패만으로 장루까지 연결되는 경로"로 읽힌다는 것.
1차 바이오피드백 → 객관적 재평가·구조병변 확인 → 전문 다학제 판단 → 정기 관장 →
**예외적 구제수술로서의 장루**로 단계를 명시했다. **한 번 고친 것도 다시 볼 것.**
