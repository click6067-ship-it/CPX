# 다음 세션 인계 — 설사·변비 발현소견 검토표 (2026-08-11 **13라운드** 종료 시점)

> 이 파일부터 읽고 시작한다. 이전 세션 전문은 `~/main/logs/`, 요약은 `docs/worklog.md` 맨 끝,
> 검수 이력 정본은 `docs/verification-log.yaml`(**13라운드까지** 기록됨).

## 지금 어디까지 됐나

**커밋·푸시 완료 (공개 repo)**
- `ontology/diarrhea.yaml` **19질환** · `ontology/constipation.yaml` **14질환** (검토표는 +【공통 경로】 카드 2장)
...
- 설사 20카드 소견71·조건24·참고90 / 변비 15카드 소견53·조건15·참고52 = **305건, 인용 대조 305/305 통과 · 구조 경고 0건**
- 바탕화면 사본 = `CPX_설사변비_온톨로지_260811/15_설사_*_v7.*`, `16_변비_*_v7.*` (v4~v6도 나란히 보존)

## 🚨 최우선 — Codex 10차 MAJOR 잔여 + 11차 검수

원문 = 바탕화면 `Codex검수10_luna.md`(4~10차 전부 보존. 입력 브리프도 각각)

**10차 판정 REVISE — BLOCKER 3 · MAJOR 24 · MINOR 6. BLOCKER 3건은 13라운드에서 반영했다.**
**MAJOR 약 18건·MINOR 6건이 남았고 11차 검수도 안 돌렸다 — 거기서부터 시작하면 된다.**
남은 것: Rome IV 정밀화 · UC 중증도 기준명 통일(Montreal vs Truelove–Witts 혼용) ·
장티푸스 카드 분리 · CDI 항생제 노출 기간(최근 8주 등) · 여행자 설사의 비혈성 치료 경로 ·
기능성 변비의 90% 수치 · 약물유발 설사 required 등.

### 🔴 이 세션의 결론 — 패턴 자체가 답이다
**10회 연속 REVISE. 5차 이후의 BLOCKER는 전부 직전 라운드의 내 수정이 만든 것이었다.**
혈성 설사 항생제 규칙만 **일곱 번** 고쳤고 매번 다른 각도의 결함이 나왔다:
혈변만으로 항생제 → 전면 금지(치료 지연) → 병렬 조건(같은 환자가 양쪽) → 4단계 우선순위 →
환자 3분기 → A/B 겹침(패혈증은 쇼크 없이도 성립) → **장기기능장애 오적용**(탈수성 쇼크·HUS를
감염성 패혈증과 동일시해 STEC+급성신손상에 항생제 지시).
**구조는 코드로 닫혔지만(빈 카드·중복·모순 0) 임상 판단은 LLM 왕복으로 수렴하지 않는다.**
**임상 권위자의 검증 없이 교육에 쓰면 안 된다.**

### ⭐ 도구 — `findings_review.py`가 구조 모순을 잡는다
같은 축의 인용 중복 · `required_없음_사유`와 required 공존 · `해당없음` 축 충돌 · required와 사유 동시 부재.
**수정 스크립트를 돌린 뒤 반드시 이 경고를 확인할 것.** 실제로 이번 세션에서 12건을 잡았다.

### ⭐ 【공통 경로】 카드 (12라운드 신설)
탈수·쇼크·항생제·지사제·입퇴원 기준은 질환 카드가 아니라 이 카드에 있다.
질환 카드에 중복하면 그래프상 "쇼크 하나로 아무 질환이나 지지"된다(Codex 9차 지적).
