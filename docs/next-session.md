# 다음 세션 인계 — 설사·변비 발현소견 검토표 (2026-08-11 **8라운드** 종료 시점)

> 이 파일부터 읽고 시작한다. 이전 세션 전문은 `~/main/logs/`, 요약은 `docs/worklog.md` 맨 끝,
> 검수 이력 정본은 `docs/verification-log.yaml`(**8라운드까지** 기록됨).
...
- 설사 소견86·조건20·참고76 / 변비 소견59·조건7·참고46 = **294건, 인용 대조 294/294 통과**
- 바탕화면 사본 = `CPX_설사변비_온톨로지_260811/15_설사_*_v7.*`, `16_변비_*_v7.*` (v4~v6도 나란히 보존)

## 🚨 최우선 — Codex **6차 검수 결과 반영**

원문 = 바탕화면 `Codex검수6_최종_luna.md`(4차·5차 것도 나란히 있음. 입력 브리프도 각각 보존)

**6차 판정 REVISE — BLOCKER 2 · MAJOR 40 · MINOR 4. BLOCKER 2건은 8라운드에서 반영했다.**
남은 **MAJOR 40 · MINOR 4가 다음 세션의 시작점**이다. 원문 = 바탕화면 `Codex검수6_최종_luna.md`.

### 🔴 이 산출물은 "완료"가 아니다
**6회 연속 REVISE**였고, **매 라운드 새 BLOCKER가 나왔다.**
그리고 5·6차의 BLOCKER는 전부 **직전 라운드의 내 수정이 만든 것**이었다.
구조는 닫혔지만(아래) 내용은 계속 틀리고 있다 — 임상 검증 없이 교육에 쓰면 안 된다.

남은 MAJOR의 성격: required/참고항목의 의미가 카드별로 흔들림 · 경쟁질환 노출과 합병증이
발현소견에 섞임 · Rome/골반저 진단기준의 정밀도 · 변 형태만으로 통과시간 판정 등.

### 구조는 닫혔다 — 이제 내용의 깊이가 남았다
7라운드 이후 **소견 2개 이하 카드 0장 · red_flag 없는 카드 0장 · required 없는 카드 0장**(사유 미기재 기준).
질환당 발현소견 설사 4.5 · 변비 4.2 (5라운드 직후 3.4 / 3.1). 참고표(복통) 18에는 아직 못 미친다.
