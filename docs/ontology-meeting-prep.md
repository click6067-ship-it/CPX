...
미팅에서 **두 사람이 서로 다른 "층(layer)"의 얘기**를 했고, 지금 그게 섞여 있다. 분리가 이해의 핵심.

| 누가 | 제안 | 기술 정체 | 우리 현황 |
|---|---|---|---|
| **박정빈 교수** | "온톨로지 + Neo4j 그래프로 LLM이 **근거 갖고** 사례 생성" | **지식그래프 기반 grounded generation (GraphRAG 계열)** | 06-18에 비용 사유로 **v2+ 이관**했던 것 → **앞당겨지는 상황** |
| **채원우** | "35B Q4 256k 로컬모델 + 마크다운 CPX 사례 + 키워드 온톨로지 + Skill.md" | **로컬 LLM + LLM wiki 패턴(Karpathy/DeepWiki)** | 안 쓰던 *인프라·지식포맷* 각도 |

**핵심: 둘은 대립이 아니라 다른 층이다.** 박정빈 = "관계 구조(지식그래프)를 어떻게 만들까", 채원우 = "지식을 어떤 포맷·어떤 모델에 담을까". 미팅에서 이게 정리 안 되면 "온톨로지를 Neo4j로 하냐 vs 마크다운으로 하냐"를 두고 헛돈다. **공통점 = 둘 다 "온톨로지(원인·증상·질환 의미설계)를 먼저"라는 데는 일치.**
...
**A. 박정빈(Neo4j KG) ↔ 채원우(마크다운 wiki) 정렬** — 대립 아니라 다른 층:
- KG/Neo4j = 관계·생성뼈대(B)·감별추론. wiki/마크다운 = 정제지식 로드. **둘 다 가능.**
- 합의안: *"온톨로지 의미설계(엔티티·관계)는 공통으로 먼저 → Neo4j 그래프로도 담고 / 마크다운으로도 내보낸다."* (채원우 말대로 설계가 먼저, 저장포맷은 그다음.)
...
**C. 차별점(교수 어필):** CPX/OSCE를 지식그래프로 **생성**하는 건 아직 논문 없음 + 부산대 실데이터 보유 → **Pattern B 그래프-스캐폴드 생성 = 신규 기여.**
...
### 10.6 미팅 최종 권고 멘트 (과대주장 없이 교수 제안 수용)
> **"CPX 온톨로지는 SNOMED 같은 표준을 참조하되, 핵심은 한국 CPX 수행항목과 환자 응답정책입니다. 먼저 1개 주증상에 대해 질환-증상-감별-체크리스트 그래프 카드를 만들고, 이 카드가 생성 사례의 누락·모순을 줄이는지 검증하겠습니다. Neo4j/GraphRAG는 그 다음 확장입니다."**
