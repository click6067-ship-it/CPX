...
> ✅ **확정 (2026-07-01, 임선주 전달):** 박정빈 교수 미팅에서 **온톨로지 적용 결정.** "Neo4j로 그래프를 그려 **구조화된 지식 내에서 LLM이 근거를 갖고 사례 생성**"(아무렇게나 X). **LLM wiki(마크다운 정제지식) · 지식그래프(Neo4j 타입관계) · RAG(큰 교과서) 3층 모두 박정빈 교수의 단일 비전**이다 — 채원우·팀 발언은 그 *전달*이지 별개 의견이 아니다(해석 오해 방지). 7/2(목) 12시 시뮬레이션센터 팀 작업, 숙제 = "CPX 엔티티를 뭘로?"(→ §2.1).
