...
> **정본 설계 계획.** 2026-06-30 초안 · **2026-07-01 교수 원문 반영·갱신**(김용하+Claude+Codex). 박정빈 교수 비전(7/2 미팅) 실행 계획.
...
> 📩 **박정빈 교수 원문 (2026-07-01, 임선주 전달 — 정본 verbatim, 모든 해석의 기준):**
> "오늘 박정빈 교수님과의 미팅에서 **온톨로지를 적용**하기로 했습니다. **Neo4j로 그래프를 그려주면 구조화된 지식 내에서 LLM이 사례를 만들 수 있다**고 합니다. LLM이 아무렇게나 사례를 만드는 것이 아니라, **근거를 가지고 사례를 만들도록** 하는 방법인 것 같습니다.
> … **7월 2일(목) 12시**, 의과대학 3층 시뮬레이션센터에서 점심 같이 먹고 작업. (원우 가능 · 민성 가능하면 · 호영 점심+틈틈이 · 용하 야간작업 시 연락 · 이혜윤 교수 가능.)
> 각자 온톨로지와 Neo4j를 살펴보고, **CPX에 온톨로지를 적용하려면 엔티티를 뭘로 해야 할지** 생각해 오기:
> - **35B Q4 · LLM wiki 처럼 · Skill.MD · context size 256k 이상 최대로 확인 · TurboQuant 가능한 버전**
> - **Markdown으로 CPX 사례 변환**
> - **Graph 연결이 아닌 온톨로지 — 원인·증상·질환**
> - **Keyword 기반 ontology 어떻게 디자인할지**"
>
> ↳ **김용하 지시(2026-07-01):** 위 교수님 의견이 **가장 중요한 정본** — 그대로 따른다. 그 토대 위에 아이디어 추가·살 붙이기는 가능(반박·대체가 아니라 충실 실행 + 보강).

**해석 (오해 방지):** 원문의 **LLM wiki(마크다운 정제지식) · 지식그래프(Neo4j 타입관계) · RAG(큰 교과서) · 로컬 35B 롱컨텍스트** 는 모두 **박정빈 교수의 단일 비전**이다 — 채원우·팀 발언은 그 *전달*이지 별개 의견이 아니다. 우리 자세 = 이 스택을 **충실히 실행**하고, 그 결과(필수요소 누락·모순·교수 accept율)를 **측정**한다. *(측정은 우리 산출물의 학술 정직성을 위한 것 — 교수님 기술선택에 대한 저항이 아니다. 비전 실행과 측정은 양립한다.)*
...
### 2.2 관계
`Disease -has_symptom→ Symptom` · `-has_sign→ Sign` · `-has_risk_factor→ RiskFactor` · `-caused_by→ Cause/Etiology` · `RiskFactor -predisposes_to→ Disease` · `-has_red_flag→ RedFlag` · `-differential_of→ Disease` · `-indicated_test→ Test` · `-treated_by→ Treatment` · `Symptom -discriminates_between→ Disease/Disease` · `ChecklistItem -assesses→ Symptom/Action`
> **원인축 (교수 원문 "원인·증상·질환"):** `RiskFactor`(소인·위험인자)와 `Cause/Etiology`(병태생리 원인)는 임상적으로 다르다. 원인을 `RiskFactor` 하나로 뭉갤지, 별도 `Cause` 엔티티 + `caused_by`/`predisposes_to` 관계로 세분할지는 **§7 교수 확인** — 교수님이 "단순 graph 링크가 아닌 원인 온톨로지"를 강조했으므로 원인축을 1급 관계로 둘 여지.
...
- **Q5 추가(검증 노동)**: *"임상 검증은 누가(교수/임선주/공동)·시간 얼마나? 그에 맞춰 흉통 검증 분량을 잡겠습니다."* — "교수-검증 그래프" 주장의 실현가능성 핵심.
