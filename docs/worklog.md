...
- **[검토표 11라운드 — Codex 8차 BLOCKER 2·MAJOR 18·MINOR 5 전량 반영]** 🚨 **BLOCKER ① 항생제 규칙을 '조건'에서 '환자 분기'로**: 7차의 4단계 우선순위도 "**'STEC 배제 후'는 검사 전에는 실행 불가능**하고 패혈증 즉시 치료 경로와 겹친다"는 지적을 받았다 → 조건 나열이 아니라 **환자를 먼저 세 갈래로**(A 쇼크·패혈증=검체 채취하되 결과 안 기다리고 즉시 소생술+항생제 / B 쇼크 아님+STEC 고위험=항생제·지사제 회피 / C 그 밖=배양+Shiga 독소 동시 검사, 경험적 치료는 중증·고위험군 선택적·전문과 판단). **이 규칙 하나를 5개 라운드에 걸쳐 네 번 고쳤다.** 🚨 **BLOCKER ② HUS 수액 3분기**: 일반 카드의 "심한 탈수면 정맥수액"과 HUS의 수액 제한이 연결 안 돼 **과잉수액·쇼크 치료 지연이 모두 가능**했다 → 쇼크면 즉시 소생 / 저혈량이면 10~20 mL/kg 신중 / 핍뇨·무뇨 HUS면 과부하 회피 + 체중·소변량·전해질·신기능 감시 + 신장내과. **MAJOR/MINOR**: 관류 평가를 discriminator→red_flag(병원체 구별이 아니라 공통 중증도 축, 9개 카드 일괄) · **경보소견 표현을 "이 진단으로 보지 않는다"→"추가 평가 전에는 단정하지 않는다"**(경보소견은 배제 판정이 아니라 평가 촉발이며 공존 가능) · UC·크론병의 required 제거(UC는 직장염~전대장염, 크론은 부위별 표현형에 따라 주증상이 달라 '거의 항상 있는' 소견이 없다) · CDI 일반형/전격형 경로 분리 + NAAT·배양 단독 진단 삭제(무증상 집락화 오진) · Shigella 균종정보를 후유증으로/실제 HUS 소견을 red_flag로 · STEC 연령을 의심도가 아니라 중증도 인자로 · 대장암 '50세+복통'을 연령 무관 경보로 · 서행성 변비 수술을 전문의 선택으로 · 배변협조장애 회음하강 수치를 보조 기준으로 · 신경질환성 변비 항문반사 단독판단 금지 · 직장류 splinting을 supporting으로 · '가성설사' 표현 제거 등. **검증: 311/311 인용대조 + 구조 경고 0건.** 미완: 9차 검수 미실시.
- **[검토표 12라운드 — Codex 9차 전량 반영 + 【공통 경로】 카드 신설]** ⭐ **핵심은 온톨로지 설계 결함 해소**: Codex 9차가 "**탈수·쇼크 소견이 모든 질환 카드의 red_flag로 중복 연결돼 그래프 가중치상 쇼크 하나로 바이러스·Campylobacter·STEC 등 아무 질환이나 지지된다**"고 지적 — 온톨로지로 사례를 생성하는 이 프로젝트 목적에 직접 해가 되는 구조 문제다. → **【공통 경로】 카드 2장 신설**(설사: 중증도·응급 처치 / 변비: 경보평가·분변매복)에 공통 항목을 모으고 질환 카드에서 **설사 37건·변비 6건 제거**, 포인터만 남김. **부수 효과: 여섯 번 고친 항생제 규칙이 한 곳에만 존재하게 되어 카드 간 불일치가 원천 차단됨.** 🚨 **BLOCKER ① 항생제 3분기 상호배타화**: 8차 판의 A("쇼크·패혈증")와 B("쇼크가 아니면서 STEC 고위험")가 겹쳤다 — 패혈증은 쇼크 없이도 성립하므로 **같은 환자에게 항생제와 회피가 동시 지시**됐다 → A=쇼크 **또는** 패혈증/장기기능장애, B=**A가 아니면서** STEC 임상상, C=그 밖(IDSA가 검사 전 경험적 치료를 허용하는 예외 — 3개월 미만·발열성 이질·발열/패혈증 여행자·중증 면역저하 — 명시, 판단은 감염내과·소아과). **이 규칙 3차~9차 여섯 번 수정.** 🚨 **BLOCKER ② HUS 수액 고정용량 제거**: "10~20 mL/kg" 일률 적용을 버리고 설사 초기 저혈량(매 bolus 후 관류·소변량·체중·입출량 재평가)과 확립된 핍뇨·무뇨 HUS(과부하 회피)를 분리. **MAJOR/MINOR**: Salmonella 항생제 금지를 "합병증 없는 면역정상 성인"으로 한정+고위험군을 배경조건으로 · Shigella에서 무발열 혈변(STEC 단서) 제거·required 축 교정 · STEC 증상 노드에서 치료 지시 분리 · CDI 전격성을 "저혈압/쇼크 또는 장마비 또는 독성거대결장"으로 명시하고 기전·위막을 참고항목으로 · **담즙산 설사 required 복원**(설사가 모두 supporting이면 담낭절제 병력만으로 진단 후보가 생성됨) · 직장류에 배변조영술 해부학 소견 추가(증상만으로 생성되던 문제) · 배변협조장애 객관적 검사 2개 요구 · 서행성 변비 내부 모순 해소 · 약물유발 변비 도파민작용제 강등. **배경조건 밀도 보강**(분만 손상·성별·연령·인종·힘주기 등, 변비 7→15·설사 19→24). **근거 미확보는 지어내지 않고 명시**(Rome IV Bristol 아형 기준, 기능성 변비의 폐쇄감·완하제·IBS 배제 조건). **검증: 302/302 인용대조 + 구조 경고 0건.** 미완: 10차 검수 미실시.
- **[검토표 13라운드 — Codex 10차 BLOCKER 3건 반영, 세션 종료]** 🚨 **BLOCKER ①② (같은 뿌리)**: 9차 판의 항생제 A 분기가 "쇼크 **또는** 장기기능장애"였는데 **탈수성 쇼크와 HUS의 급성신손상도 여기 걸린다** — 그 환자들은 항생제 적응이 아니고 STEC면 오히려 해롭다. 그래서 **STEC+급성신손상 환자에게 '항생제 금기'와 '즉시 항생제'가 동시에 지시**됐다. → Harrison 패혈증 정의("**감염**에 대한 조절되지 않은 반응으로 생긴 생명 위협 장기기능장애")를 근거로 A의 방아쇠를 **감염성 패혈증**으로 한정하고 "탈수성 쇼크·HUS 급성신손상은 A가 아니다"를 명시, STEC 카드에도 교차 참조를 1급 항목으로. 🚨 **BLOCKER ③**: 변비 공통 경로에 **폐색·장마비 응급 경로가 아예 없었고 분변매복도 확인만 하고 처치가 없었다** → "변과 가스가 모두 안 나오면(obstipation) 장폐색 → 즉시 영상·외과 협진" red_flag와 "매복은 수지 제거·관장으로 처치, 폐색 의심 시 완하제 회피"를 추가. **MAJOR 일부**: 항문반사 소실을 supporting→**신경학적 red_flag**(마미증후군) · 내치핵의 문맥고혈압 분리(직장정맥류는 병태가 다름) · melanosis coli 중복 제거 · 탈수 평가를 공통 경로로. **검증 305/305 + 구조 경고 0.** **⚠️ 세션 결론: 10회 연속 REVISE, 5차 이후 BLOCKER는 전부 직전 라운드의 내 수정이 만든 것. 혈성 설사 항생제 규칙만 일곱 번 고쳤고 매번 다른 각도의 결함이 나왔다. 구조는 코드로 닫혔으나 임상 판단은 LLM 왕복으로 수렴하지 않는다 — 임상 권위자 검증 없이 교육에 쓰면 안 된다.** 미완: MAJOR 약 18·MINOR 6 + 11차 검수.
- **[검토표 14라운드 — Codex 10차 잔여 MAJOR 18·MINOR 6 전량 반영]** **분기 논리 명시**: "패혈증 여행자 설사가 A와 C 예외에 동시에 걸린다"는 지적에 대해, 실제로는 **분기 순서가 곧 논리**(A면 거기서 끝, A가 아닐 때만 B, A·B가 아닐 때만 C)라는 점을 규칙 안에 못박음. **required 재정비**: 여행자 설사(지역·시점 모두 고정 조건 아님)·약물유발 설사(시간적 연관만으로 인과 확정 불가)의 required 해제 + 사유 명시, 반대로 IBS-D의 **Rome 기간 조건은 supporting→required**로 승격(빠지면 조기 진단 가능). **치료 경로 보강**: 공통 경로가 **혈성·염증성 설사만** 다뤄 비혈성 여행자 설사 분기가 닫혀 있지 않던 것을 해당 카드에 연결, 이질 카드의 'STEC 배제 후에만' 잔재 제거(중증 이질 치료 지연 + 공통 A와 충돌). **표현·수치**: '하루 3회를 넘는'→'**3회 이상**'(정확히 3회 환자를 제외하던 문제, 전 카드) · CDI 항생제 노출 기간(대개 8주)·크레아티닌 병기 · UC 중증도를 **Montreal 하나로 통일**(Truelove–Witts 혼용 제거) · 기능성 변비 '90% 초과'를 참고 통계로 격하 · 묽은 변 해석에 **넘침설사 배제 선행** 명시 · 중증 저칼륨혈증에 급성 장마비·가성폐색 · 아전결장절제 '권고'→'전문 다학제 평가 후 고려' · Salmonella 세포독소 기전 문구 교정 · 갑상선기능항진증 성별 축 자기모순 제거. **검증 304/304 + 구조 경고 0.** 11차 검수 진행.
- **[검토표 15라운드 — Codex 11차 BLOCKER 2·MAJOR 14·MINOR 8 전량 반영]** 🚨 **BLOCKER: STEC 판정을 첫 관문으로 올림(항생제 규칙 여덟 번째 수정)**. 지금까지 감염성 패혈증(A)을 먼저 보고 STEC(B)를 나중에 봤는데, **HUS·급성신손상·저혈압이 있으면 그 STEC 환자가 A로 올라가 항생제가 재투여**된다. qSOFA도 **예후 지표이지 패혈증 진단 기준이 아니고** 탈수·HUS로도 올라간다 → 1단계에서 STEC 의심부터 판정하고 "HUS·AKI·저혈압이 있어도 회피 고정(그 장기기능장애는 패혈증 반응이 아니라 독소·혈전미세혈관병), **별도의 치료 가능한 감염원이 명확할 때만** 전문과 판단으로 예외". ⭐ **검사값·합병증을 발현소견에서 분리(`kind=complication` 신설)**: "HUS·AKI·Hb/ESR/WBC/Cr가 red_flag에 섞여 **그래프가 HUS나 빈혈만으로 원인 질환을 연결**한다"는 지적(9차 공통경로 문제와 같은 계열) → HUS 진단 정의는 합병증 축으로, **관찰 가능한 소견(핍뇨·창백·멍·부종·의식변화)만 red_flag**로 신설. **required 정합성**: 여행자 설사·신경질환성 변비는 required **복원**(각각 '급성 설사가 있어야 하는 증후군', '신경계 병변이 정의적 조건'), 정착 후 독소생성·전구성 독소는 잠복기·음식을 required에서 **해제**(30분~8시간 유효 발병이나 음식 미확인 환자를 배제하던 문제). **MINOR**: 탈수 평가 지표 우열을 **연령별로**(소아는 피부긴장도·모세혈관 재충만이 표준) · 직장수지검사를 "**직장 내** 매복 확인"으로 한정(근위부는 배제 못 함) · obstipation을 "강하게 시사"로 완화 · 기능성 변비의 "묽은 변=빠른 통과" **삭제**(넘침설사 가능) · 완하제를 PEG 경구 우선/관장·좌약은 구제요법 · 대장암 내시경을 폐색 없을 때로 한정(폐색이면 CT·외과) · **메틸날트렉손에 기계적 폐색 금기**(천공 위험) · IBS 복부압통을 종괴·복막자극·국소 심한 압통으로 한정 · Shigella HUS를 S. dysenteriae 1형 한정 · 크론 분변성 구토를 위-대장/십이지장-대장 누공 한정. **검증 306/306 + 구조 경고 0.**
- **[검토표 16라운드 — Codex 12차 BLOCKER 2·MAJOR 17·MINOR 5 반영]** 🚨 **BLOCKER ① 순서가 틀렸다**: 11차에서 "STEC 의심을 가장 먼저"로 고쳤는데 **쇼크·저관류 환자에서는 원인 감별이 ABC·소생술보다 앞설 수 없다**(STEC·탈수·HUS·패혈증은 초기 임상상이 겹침) → **0단계에 무조건 ABC·관류·수액 소생술**을 두고 배양·Shiga 독소·O157 검사를 **동시에**. 🚨 **BLOCKER ② 내가 반대 방향으로 과교정**: "HUS·AKI·저혈압이 있어도 항생제 판단은 바뀌지 않는다"가 **실제 패혈증에서 생명구조 항균제를 보류**하게 만들었다(저혈압은 탈수·HUS·패혈증 어디서든 생긴다) → ⓐSTEC/HUS만 있고 패혈증 증거 없으면 회피 ⓑ**패혈증·균혈증·장외감염 의심이면 보류하지 않음**(전문과 즉시 판단) ⓒ그 밖 원칙적 비투여+정의된 예외. "STEC **의심 아님**은 **배제됨**이 아니다"도 명시. **이 규칙 3차~12차 아홉 번 수정.** ⭐ **발현소견 순도 대청소(세 번째)**: 상태 진단명(쇼크·중증탈수·장폐색·분변매복·감돈탈출·점액부종혼수·중증고칼슘·중증저칼륨·마미증후군·크론 협착/누공·부분폐색)을 전부 `complication`으로 옮기고 **관찰 소견만** 발현소견에 남김(빈맥·모세혈관 재충만 지연·obstipation·직장 내 단단한 변괴·의식저하·저체온·요폐·안장부 감각저하 등). 검사값(철결핍빈혈·혈색소·ESR·배변조영술·직장수지검사)은 `workup`, 대장암 가족력·연령은 배경조건, 올메사르탄은 복약력, PSC·괴저농피증은 관찰 표현으로. **ORS 안전 조건**: "즉시 경구수액"이 쇼크·의식저하·장마비·흡인 위험에서 정맥수액을 지연시켜 → "안전하게 삼킬 수 있는 경증·중등도 탈수에만, 그 외 등장성 정맥수액 우선". ⚠️ **3차 BLOCKER(배변협조장애 회장루)가 12차에 재지적** — 1차 바이오피드백 → 객관적 재평가 → 전문 다학제 → 정기 관장 → 예외적 구제수술로 단계 명시. **한 번 고친 것도 다시 봐야 한다.** **검증 315/315 + 구조 경고 0.** **판정은 'REVISE — SHIP 불가', 12회 연속.**
