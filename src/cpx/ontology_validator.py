...
def _default_synonyms() -> dict[str, tuple[str, ...]]:
    ...
    return {
        ...
        # ── 설사 (2026-08-12 신설) ─────────────────────────────────────────
        # ⚠️ 라벨이 "만성·재발성 설사(4주 초과)"처럼 길면 토큰 부분일치가 LOW 로 떨어져
        #    positive_coverage 가 0% 로 나온다(실측). 임상 표면형을 직접 등록해 해결한다.
        "chronic_recurrent_diarrhea": ("만성 설사", "반복", "재발", "몇 달", "수개월", "오래", "계속 설사"),
        "acute_watery_diarrhea": ("물설사", "묽은 변", "물 같은", "수양", "설사"),
        "abdominal_pain_with_diarrhea": ("복통", "배가 아프", "배 아프", "아랫배", "쥐어짜는 듯한 배"),
        "pain_relieved_by_defecation": ("배변 후", "변을 보고 나면", "화장실 다녀오면", "대변 보면"),
        "no_nocturnal_symptoms": ("자다가", "야간", "밤에는", "새벽"),
        "stress_related_symptoms": ("스트레스", "긴장", "불안", "예민"),
        "alternating_constipation": ("변비와", "변비가 번갈", "변비도"),
        "no_weight_loss": ("체중 변화", "몸무게", "체중은"),
        "bloody_diarrhea": ("혈변", "피가 섞", "피 섞", "붉은 변", "선홍색"),
        "inflammatory_diarrhea": ("혈변", "발열", "열이 나"),
        "high_fever": ("고열", "열이", "발열"),
        "severe_abdominal_pain": ("심한 복통", "몹시 아프", "참기 힘든"),
        "fecal_leukocytes": ("대변 백혈구", "변 백혈구"),
        "vomiting_prominent": ("구토", "토했", "게워"),
        "fever_present": ("발열", "열이 나", "미열"),
        "outbreak_contact": ("같이 먹은", "집단", "주변 사람", "가족도"),
        "recent_travel_history": ("여행", "출장", "해외", "다녀왔"),
        "recent_antibiotic_use": ("항생제", "약을 먹은", "처방받은 약"),
        "healthcare_exposure": ("입원", "요양", "병원에"),
        "crampy_abdominal_pain": ("쥐어짜", "경련", "쥐가 나듯", "배가 뒤틀"),
        "weight_loss": ("체중이 줄", "살이 빠", "몸무게가 줄"),
        "symptoms_after_dairy": ("우유", "유제품", "요구르트", "치즈"),
        "bloating_flatus": ("배에 가스", "더부룩", "복부팽만", "방귀"),
        "resolves_with_fasting": ("굶으면", "금식", "안 먹으면"),
        "heat_intolerance": ("더위", "더운 걸 못"),
        "palpitations": ("두근", "심장이 뛰"),
        "unintentional_weight_loss": ("체중이 줄", "살이 빠"),
        "dehydration_signs": ("갈증", "입이 마르", "목이 마르", "어지럽"),
        "overtly_bloody_stool": ("혈변", "피가 섞", "붉은 변"),
        "fever_over_38_5": ("38", "고열"),
        "elderly_or_immunocompromised": ("고령", "면역", "항암"),
        "temporal_relation_to_drug": ("약을 먹고", "복용 후", "약 시작"),
        "ask_stool_frequency": ("몇 번", "횟수", "하루에"),
        "ask_stool_form": ("변 형태", "모양", "묽", "물 같"),
        "ask_blood_or_color": ("피", "혈변", "색깔", "검은"),
        "ask_fever_chills": ("열", "오한", "춥"),
        "ask_thirst_dehydration": ("갈증", "물을", "탈수", "소변량"),
        "ask_food_relation": ("음식", "드신", "먹은 것", "식사"),
        "ask_recent_travel": ("여행", "출장", "해외"),
        "ask_medication_hx": ("약", "복용", "드시는"),
        "ask_antibiotic_hx": ("항생제",),
        "ask_weight_loss": ("체중", "몸무게", "살이"),
        "ask_stress_relation": ("스트레스", "긴장"),
        "ask_dairy_relation": ("우유", "유제품"),
        "ask_pain_with_diarrhea": ("복통", "배가 아프", "통증"),

        # ── 변비 (2026-08-12 신설) ─────────────────────────────────────────
        "infrequent_bowel_movement": ("주 3회", "일주일에", "며칠에 한 번", "배변 횟수"),
        "hard_lumpy_stool": ("딱딱", "굳은 변", "토끼똥", "덩어리"),
        "excessive_straining": ("힘을 주", "힘주기", "용을 쓰", "힘들게"),
        "incomplete_evacuation": ("잔변", "다 못 본", "덜 본 느낌", "시원하지"),
        "vaginal_pressure_to_defecate": ("질 뒤", "질을 누르", "회음"),
        "digital_maneuver_to_evacuate": ("손가락", "손으로 파", "빼내"),
        "protruding_anal_mass": ("빠져나", "튀어나", "덩이", "탈출"),
        "painful_defecation": ("배변 시 통증", "볼 때 아프", "항문이 아프"),
        "painless_bright_red_bleeding": ("아프지 않", "무통", "선홍색"),
        "recent_onset_constipation": ("최근", "얼마 전부터", "몇 달 전부터"),
        "ask_bowel_frequency": ("몇 번", "횟수", "일주일"),
        "ask_stool_hardness": ("딱딱", "굳", "단단"),
        "ask_straining": ("힘", "용", "애쓰"),
        "ask_incomplete_evacuation": ("잔변", "시원", "다 못"),
        "ask_digital_or_enema_use": ("손가락", "관장", "빼내"),
        "ask_blood_in_stool": ("피", "혈변", "출혈"),
        # 경보징후(공통 경로) — 학생이 "선별했는가"로 잡히도록 표면형을 넓게 둔다
        "alarm_features_chronic_diarrhea": ("혈변", "체중", "빈혈", "야간", "밤에", "발열"),
        "constipation_alarm_features": ("혈변", "체중", "빈혈", "발열", "구토", "가족력"),
        "cauda_equina_syndrome": ("소변", "요폐", "감각", "저림", "다리 힘"),
        "hemolytic_uremic_syndrome": ("소변량", "creatinine", "핍뇨", "창백", "멍"),
        "toxic_megacolon": ("복부팽만", "배가 부풀", "장음"),
        "bowel_obstruction_sign": ("가스", "방귀", "복부팽만", "구토"),
        "fecal_impaction": ("변이 막", "딱딱한 변", "직장"),
        "megacolon": ("거대결장", "복부팽만"),
        "alarm_weight_loss": ("체중", "몸무게", "살이"),
        "alarm_rectal_bleeding": ("혈변", "출혈", "피"),
        "alarm_anemia": ("빈혈", "창백", "어지"),
    }
