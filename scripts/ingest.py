...
def to_cpxcase(text: str, case_id: str, model: str | None = None) -> CpxCase:
    # ingest=품질 작업. 과부하(503) 대비 모델 폴백 체인.
    chain = [model] if model else ["gemini-2.5-flash", "gemini-2.5-flash-lite", "gemini-flash-lite-latest"]
    prompt = PROMPT.format(text=deid(text)[:40000])
    last = None
    for m in chain:
        try:
            return llm.complete_json(prompt, CpxCase, model=m).model_copy(update={"case_id": case_id})
